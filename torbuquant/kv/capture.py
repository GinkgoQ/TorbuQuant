...
class RingBuffer:
    ...
    def total_written(self) -> int:
        """Total tokens ever written (including overflowed)."""
    ...
    def memory_bytes(self) -> int:
        """Estimate GPU memory used by buffer."""
        elem_size = self._k.element_size()
        return 2 * self.capacity * self.num_kv_heads * self.head_dim * elem_size
...
class KVCaptureEngine:
    ...
    def total_compressed_tokens(self) -> int:
        """Number of tokens in compressed store."""
        return self.store.num_tokens

    @property
    def total_buffered_tokens(self) -> int:
        """Number of tokens in ring buffer."""
        return self.ring.size

    @property
    def total_tokens(self) -> int:
        """Total tokens (compressed + buffered)."""
    ...
    def memory_bytes(self) -> dict:
        ...
        return {
            "ring_buffer": self.ring.memory_bytes(),
            "compressed_store": self.store.memory_bytes(),
            "total": self.ring.memory_bytes() + self.store.memory_bytes(),
        }
