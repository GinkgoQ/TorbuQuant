...
Responsibilities:
  ...
  - Keep patching surface tiny; all real logic lives in capture/store
...
class LayerState:
    ...
    config: LayerConfig
    store: object  # CompressedKVStore
    engine: object  # KVCaptureEngine
    _log_count: int = 0
...
def _is_mla_impl(impl) -> bool:
    """Check if implementation is MLA (Multi-head Latent Attention)."""
    return (
...
def _no_alloc_prefill_attention(
    ...
    """Compute attention without using paged cache."""
        ...
        if mode == MODE_HYBRID and state.supports_hybrid:
            ...
            if flat is not None and flat.num_tokens >= 16:
                ...
                result = compute_hybrid_attention(
                    query=q,
                    flat_cache=flat,
                    key_compressor=state.store.key_compressor,
                    value_compressor=state.store.value_compressor,
                    recent_k=recent_k,
                    recent_v=recent_v,
