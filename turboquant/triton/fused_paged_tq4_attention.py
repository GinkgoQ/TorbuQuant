...
This kernel reads TQ4-compressed blocks directly from vLLM's paged block table,
decompresses in SRAM (nibble unpack -> centroid gather -> norm scale), and
computes FP16 Q@K^T with online softmax in a single fused pass.

No HBM writes of decompressed cache -- HBM traffic drops from 1,160 to
136 bytes/token (8.5x reduction).
...
# BN=16 dropped: consistently slowest across 1K-32K
# ---------------------------------------------------------------------------
...
def _fused_paged_tq4_decode_kernel(
    ...
    # ── Dual-path switch ──
    USE_INT8_QK: tl.constexpr = False,
    QJL_DIM: tl.constexpr = 0,
    # ── Tiling (autotuned) ──
